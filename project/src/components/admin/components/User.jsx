...
const User = () => {
  const URL = "http://localhost:3000/user"; // URL của API
  ...
  const onChange = (e) => {
    ...
    const filteredUsers = users.filter(user => user.nameRegister.toLowerCase().includes(searchValue.toLowerCase()));
  ...
  useEffect(() => {
    const updateColumnWidths = () => {
      ...
      const firstRowCells = contentRef.current.querySelectorAll('tr:first-child td');
  ...
  return (
    <div>
      ...
      <div className="table-container">
        <Input placeholder='Tìm tên người dùng...' className='search-movie-admin' value={value} onChange={onChange} />
            ...
            <tbody>
              {searchProduct.map((user, index) => (
                <tr key={user.id}>
                  ...
                  <td>
                    {
                      user.nameRegister != "Administrator" && 
                      <Button 
                      type="primary" 
